  ...
  function categoriseCourses(courseArr) {
    ...
    for (const item of courseArr) {
      if (!countedCategory.includes(item.clid)) {
        const currentItemCourses = courseArr.filter(
          (curItem) => curItem.clid === item.clid
        );
        countedCategory.push(item.clid);
        let filteredCourse = coursetypelist.filter(
          (obj) => obj.id === item.clid
        );
                  ...
                  <div className="collegeView__list">
                    ...
                    {item.courses.map((courseItem) => {
                      return (
                        <Link
                          key={courseItem.cid}
                          className="collegeView__listItemLink"
                          to={"/Course/detail/" + courseItem.cid}
                        >
        ...
        children={[
          <AccordionChild key={Math.random()}>
            <AccordionChildItem
              index={0}
              title={
                "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Obcaecati veritatis velit inventore optio error voluptate voluptatibus quasi quo alias, exercitationem laborum itaque laboriosam animi voluptates facere beatae consequatur et corrupti."
              }
            >
              <div></div>
            </AccordionChildItem>
